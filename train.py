...
if __name__ == "__main__":
    ...
    if model_path != '':
        ...
        # ------------------------------------------------------#
        #   根据预训练权重的Key和模型的Key进行加载，并且显示出成功加载的参数层
        # ------------------------------------------------------#
        ...
        for k, v in pretrained_dict.items():
            ...
            if k in model_dict.keys() and np.shape(model_dict[k]) == np.shape(v):
                temp_dict[k] = v        # 记录相应key层的权重，字典格式
                load_key.append(k)
            else:
    ...
    # ----------------------#
    if local_rank == 0:
        # 初始化一个自定义的history对象，包括summarywriter对象和log文件，并且记录loss
    ...
    # 开始训练，先准备CUDA和相关的数据集并且显示配置
    model_train = model.train()             # 创建网络实例
    # ----------------------------#
    #   是否单机多卡同步Bn训练
    # ----------------------------#
    ...
    if local_rank == 0:
        ...
        # ----------------------------------------------------------#
        wanted_step = 3e4 if optimizer_type == "sgd" else 1e4           # 理想的训练总次数
        total_step = num_train // batch_size * Epoch        # 数据集训练的总次数
        if total_step <= wanted_step:
            wanted_epoch = wanted_step // (num_train // batch_size) + 1         # 理想的epoch数
