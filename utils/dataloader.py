...
class SiameseDataset(Dataset):  # 创建自己的dataset类型
    def __init__(self, input_shape, lines, labels, random, autoaugment_flag=True):
        self.input_shape = input_shape
        self.train_lines = lines
        self.train_labels = labels
        self.types = max(labels)        # 等于最大标签值

        self.random = random            # 是否进行随机变换，测试集一般不需

        self.autoaugment_flag = autoaugment_flag
        if self.autoaugment_flag:               # 自动变换图像的规则参数，一些transform规则加自定义组合
            self.resize_crop = RandomResizedCrop(input_shape)
            self.policy = ImageNetPolicy()          # 色域变换

            self.resize = Resize(input_shape[0] if input_shape[0] == input_shape[1] else input_shape)
            self.center_crop = CenterCrop(input_shape)

    def __len__(self):
        return len(self.train_lines)            # 返回的图片数，作为dataloader的判断标准

    def __getitem__(self, index):
        # 只接受一个索引值作为输入，遍历dataloader时，会自动传入索引，因此需要提前组织好训练时每一个批次读入的数据组格式
        batch_images_path = []
        # ------------------------------------------#
        #   首先选取三张类别相同的图片
        # ------------------------------------------#
        c = random.randint(0, self.types - 1)               # 随机选取一个数（类别），例如0~8中选取一个数
        selected_path = self.train_lines[self.train_labels[:] == c]         # 先取出取出所有类别为该类的图片索引，最后通过这个索引列表返回图片路径的列表
        while len(selected_path) < 3:               # 如果图片数量小于3，去另一个类别---------------------这个方法对于one-shot来说有bug
            c = random.randint(0, self.types - 1)
            selected_path = self.train_lines[self.train_labels[:] == c]

        image_indexes = random.sample(range(0, len(selected_path)), 3)      # 随机取样，选择3张同类图片的索引
        # ------------------------------------------#
        #   取出两张类似的图片
        #   对于这两张图片，网络应当输出1
        # ------------------------------------------#
        ...
        #   取出与当前的小类别不同的类
        # ------------------------------------------#
        different_c = list(range(self.types))                   # 这时候有0~9类进行选择
        different_c.pop(c)                                          # 弹出‘9’类（必不同）
        ...
        batch_images_path.append(selected_path[image_indexes[0]])       # 取出另一张
    ...
    def _convert_path_list_to_images_and_labels(self, path_list):
        ...
        #   len(path_list) / 2  = 2
        # -------------------------------------------#
        number_of_pairs = int(len(path_list) / 2)           # 训练对的长度
        # -------------------------------------------#
        #   定义网络的输入图片和标签的形状                      ？？？？？？？？？？？？？？
        # -------------------------------------------#
        ...
        for pair in range(number_of_pairs):
            ...
            image = cvtColor(image)
            if self.autoaugment_flag:
                image = self.AutoAugment(image, random=self.random)             # 图片随即变换
            else:
                image = self.get_random_data(image, self.input_shape, random=self.random)
            image = preprocess_input(np.array(image).astype(np.float32))
            image = np.transpose(image, [2, 0, 1])
